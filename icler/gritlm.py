...
from e5CausalLM import E5ForCausalLM, E5ForCausalLMWithAttn
...
class GritLM(torch.nn.Module):
        ...
        **kwargs, # Passed to the model, e.g. `attn_implementation`, `torch_dtype` etc.
        ...
        if mode == 'embedding':
            if any([x in model_name_or_path for x in ['gtr', 't5', 'instructor']]):
                # print("sfdjasfjjajfiafieo--12---1-23--")
            ...
            if 'e5' in model_name_or_path.lower():
                ...
                self.model = E5ForCausalLMWithAttn.from_pretrained(model_name_or_path, **kwargs)
                self.generate = self.model.generate
                # E5CausalLM的编码器输出直接来自model
                self.embedding_attr = None
            else:
                # 原有的其他生成模型的代码
                self.model = AutoModelForCausalLM.from_pretrained(model_name_or_path, trust_remote_code=True, **kwargs)
    ...
    def save_pretrained(self, save_directory):
        ...
        # 保存模型
        model_save_path = os.path.join(self.output_dir, "pytorch_model.bin")
